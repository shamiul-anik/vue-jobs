...
vi.mock('../../services/api.js', () => ({
  jobsAPI: {
    addJob: vi.fn(() => Promise.resolve({ id: 1, message: 'Job created successfully' })),
  },
}));
...
describe('AddJobView.vue', () => {
  ...
  it('displays form title', () => {
    ...
    expect(text.includes('add') || text.includes('new') || text.includes('job')).toBe(true);
  });
  ...
  it('has title input field', () => {
    ...
    const titleInput = wrapper.find('input[name="title"]') || wrapper.find('input[type="text"]');
    expect(titleInput.exists() || wrapper.text().toLowerCase().includes('title')).toBe(true);
  });
  ...
  it('has job type select field', () => {
    ...
    const typeSelect = wrapper.find('select') || wrapper.find('input[name="type"]');
    expect(typeSelect.exists() || wrapper.text().toLowerCase().includes('type')).toBe(true);
  });
  ...
  it('has description textarea field', () => {
    ...
    expect(textarea.exists() || wrapper.text().toLowerCase().includes('description')).toBe(true);
  });
  ...
  it('has contact email input field', () => {
    ...
    expect(wrapper.text().toLowerCase().includes('email') || wrapper.text().toLowerCase().includes('contact')).toBe(true);
  });
  ...
  it('has submit button', () => {
    ...
    const submitButton = wrapper.find('button[type="submit"]') || wrapper.findAll('button').find(btn => 
      btn.text().toLowerCase().includes('submit') || btn.text().toLowerCase().includes('post')
    );
